...
const navGroups: { label?: (t: Dict) => string; items: NavItem[] }[] = [
  ...
  },
  {
    ...
    items: [
      { href: "/growth", label: (t) => t.nav.growth, icon: TrendingUp, ownerOnly: true },
      { href: "/money", label: (t) => t.nav.money, icon: Wallet, ownerOnly: true },
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const visibleNavGroups = navGroups
    .map((group) => ({
      ...group,
      items: group.items.filter((item) => !item.ownerOnly || account?.kind === "owner"),
    }))
    .filter((group) => group.items.length > 0);
  const flatItems = visibleNavGroups.flatMap((group) => group.items);
        ...
        <header className="mobile-app-header sticky top-0 z-20 border-b border-border bg-background/90 backdrop-blur md:hidden">
          <div className="flex h-14 items-center gap-3 px-4">
            <div className="min-w-0 flex-1">
              <div className="text-sm font-semibold">{account?.displayName || t.nav.appName}</div>
            </div>
          </div>
