...
/**
 * 「今日」页的单日时间轴：与周计划共用 timeline-shared 的网格数学和 server actions，
 * 只渲染今天一列（例行 + 今日工作块 + 当日出差全天条 + 现在线）。支持竖向拖动/拉伸/点击编辑/右键删除。
 */
export function TodayTimeline({
  ...
  function openMenu(e: React.MouseEvent, block: WeekBlock) {
    ...
    setMenu({
      ...
      y: e.clientY,
      items: [
        { label: t.week.edit, onClick: () => setEditing(block) },
        {
          label: t.week.delete,
          danger: true,
          onClick: () => {
    ...
    <div className="today-timeline min-w-0 w-full overflow-hidden rounded-xl border border-border bg-card shadow-sm">
      ...
      {tripTitles.length ? (
        <div className="flex flex-wrap items-center gap-2 border-b border-border px-4 py-2">
          <span className="text-[10px] text-muted-foreground">{t.week.allDay}</span>
          {tripTitles.map((title) => (
            <span
              key={title}
              title={title}
              className="truncate rounded bg-violet-500/15 px-2 py-0.5 text-[11px] font-medium text-violet-700"
            >
              {title}
            </span>
          ))}
          ...
          {items.map((block) => {
            ...
            return (
              ...
              >
                ...
                {block.location && height > 46 ? (
                  <div className="truncate text-[10px] opacity-70">📍{block.location}</div>
                ) : null}
                <span
