...
export function TaskIntakePanel({
  ...
  async function readScreenshot(file: File) {
    ...
    try {
      ...
      setMessage("截图文字已在本机识别，可继续修改后整理。");
    } catch {
  ...
  async function commit() {
    ...
    try {
      ...
      setMessage(
        `已新建 ${data.created} 项任务，排入周计划 ${data.scheduled} 项${
          data.skipped ? `，跳过重复 ${data.skipped} 项` : ""
        }。`,
      );
      setItems([]);
      setInput("");
  ...
  return (
    <section className="rounded-lg border border-border bg-card p-5 shadow-sm">
      ...
      <div className="grid gap-3 md:grid-cols-[minmax(180px,0.8fr)_160px_minmax(0,2fr)]">
        <label>
          <span className="flabel">所属项目</span>
          <select value={projectId} onChange={(event) => setProjectId(event.target.value)} className="field">
            <option value="">个人 / 内部事务</option>
        ...
        <label>
          <span className="flabel">我做了什么 / 接下来要做什么</span>
          <textarea
            value={input}
            onChange={(event) => setInput(event.target.value)}
            className="field min-h-24 resize-y"
            placeholder="例如：询问樊老师缪子问题是否完成；整理反馈并翻译英文；发给 Muru"
          />
        </label>
      </div>

      <div className="mt-3 flex flex-wrap items-center gap-3">
        <Button type="button" onClick={analyze} disabled={!input.trim() || analyzing || ocrProgress !== null}>
          {analyzing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}
          {analyzing ? "正在整理..." : "AI 整理"}
        </Button>
        <span className="text-xs text-muted-foreground">截图仅在本机识别，任务文字在点击整理后发送给已配置的 AI。</span>
      </div>
      ...
      {items.length > 0 ? (
        <div className="mt-5 space-y-2">
          {items.map((item, index) => (
            <div key={`${item.title}-${index}`} className="grid gap-2 border-b border-border py-3 md:grid-cols-[minmax(0,1fr)_120px_110px_36px]">
              <input value={item.title} onChange={(event) => updateItem(index, { title: event.target.value })} className="field field-sm" aria-label={`任务 ${index + 1} 标题`} />
              <select value={item.priority} onChange={(event) => updateItem(index, { priority: event.target.value as PriorityValue })} className="field field-sm" aria-label={`任务 ${index + 1} 优先级`}>
                <option value="HIGH">高优先</option>
                <option value="MEDIUM">中优先</option>
                <option value="LOW">低优先</option>
              </select>
              <select value={item.minutes} onChange={(event) => updateItem(index, { minutes: Number(event.target.value) })} className="field field-sm" aria-label={`任务 ${index + 1} 时长`}>
                {[30, 45, 60, 90, 120, 180].map((minutes) => <option key={minutes} value={minutes}>{minutes} 分钟</option>)}
              </select>
              <Button type="button" size="icon" variant="ghost" title="移除" onClick={() => setItems((current) => current.filter((_, itemIndex) => itemIndex !== index))}>
                <Trash2 className="h-4 w-4" />
              </Button>
            </div>
          ))}
          <div className="flex justify-end pt-2">
            <Button type="button" onClick={commit} disabled={!dbReady || committing}>
              {committing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />}
              {committing ? "正在写入..." : "写入看板并排进周计划"}
            </Button>
          </div>
        </div>
      ) : null}
